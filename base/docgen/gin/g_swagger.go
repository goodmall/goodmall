...
// + by yiqing
var controllerPackageList [][2]string

func AddControllerPackage(localName string, pkgpath string) {
	controllerPackageList = append(controllerPackageList, [2]string{localName, pkgpath})
	//
	ParsePackagesFromDir(GetPackageRealPath(pkgpath))
}
...
func GenerateDocs(curpath string) {
	...
	*/
	apiHome := filepath.Join(curpath, "main.go") // TODO 此处写死了 后续改为外部传参设置
	f, err := parser.ParseFile(fset, apiHome, nil, parser.ParseComments)
	if err != nil {
		beeLogger.Log.Fatalf("Error while parsing %s: %s", apiHome, err)
	}
	// ------------------  modified by yiqing   -------------      ++> |
	...
	for _, cp := range controllerPackageList {

		localName := ""

		localName = cp[0]

		analyseControllerPkg(path.Join(curpath, "vendor"), localName, cp[1])
...
// by yiqing
func GetPackageRealPath(pkgpath string) string {
...
func analyseControllerPkg(vendorPath, localName, pkgpath string) {
	beeLogger.Log.Infof("临时：%s %s ", localName, pkgpath) // 临时
					...
					if specDecl.Tok == token.TYPE {
						for _, s := range specDecl.Specs {
							...
							case *ast.StructType:
								...
								beeLogger.Log.Infof("临时：控制器： %s %s ", s.(*ast.TypeSpec).Name.String(), specDecl.Doc.Text()) // 临时
								_ = tp.Struct
...
func parserComments(f *ast.FuncDecl, controllerName, pkgpath string) error {
	...
	//beeLogger.Log.Infof(" 方法： %s ", func) // 临时
	...
	beeLogger.Log.Infof("[%s ] route Paht", routerPath)
	...
	if routerPath != "" {
		...
		for _, hm := range strings.Split(HTTPMethod, ",") {
			...
			beeLogger.Log.Infof("item %#v", item)
		}
		// beeLogger.Log.Infof("[%s %s ]  %v  opt: %#v ", pkgpath+controllerName, routerPath, item, opts)
